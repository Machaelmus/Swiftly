...
@post_routes.route('/api/posts/<int:id>')
def getOnePost(id):
    onePost = Post.query.get(id)
    oneUserForThatPost = User.query.filter(User.id == Post.userId).first()
    return {'post': onePost.to_dict(), 'user': oneUserForThatPost.to_dict()}
